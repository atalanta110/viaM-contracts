...
const { ConsoleReporter } = require('relay-compiler');
...
contract('HolyPool (deposit and withdraw scenarios)', function (accounts) {
  ...
  it('HolyHand should be able to process fees when set (deposit/withdraw)', async function() {
    ...
    await this.holyhand.claimFees(this.mockusdc.address, web3.utils.toBN('700000000'), { from: accounts[5] });
  ...
  it('HolyHand should be able to process fees when set (deposit/exchange/withdraw) (DAI)', async function() {
    ...
    // yield distributor should be able to claim fees
    await this.holywing.claimFees(this.mockusdc.address, web3.utils.toBN('2999999'), { from: accounts[5] });
    await this.holyhand.claimFees(this.mockusdc.address, web3.utils.toBN('1740000'), { from: accounts[5] });
  ...
  it('HolyHand should be able to process fees when set (deposit/exchange/withdraw) (ETH)', async function() {
    ...
    console.log("NOTE: this requires 83.333 eth, so not suitable for several runs without chain restart");
